...
import { UsuarioStore } from "../../../services/usuario/usuario.store";
...
export class AgendamentoFormComponent implements OnInit {
    ...
    submit(): void {
        ...
        if (agendamento) {
            ...
            if (this.authStore.ehGestor() && status !== null && status !== agendamento.status) {
        ...
        this.store
            .add({
                data,
                horario,
                usuarioId: this.authStore.ehGestor() ? Number(usuarioId) : (this.authStore.usuarioAtual()?.id ?? 0),
                servicoId: Number(servicoId),
            })
            .subscribe((criado) => {
                if (criado) {
